...
var cluster = require('cluster'),
    ...
    LogplexLogger = require('./logplexlogger');
...
function Master(config) {
    ...
    var logger;
    if (config.server.accessLog.indexOf("syslog:") === 0) {
        logger = new LogplexLogger(config.server.accessLog);
    } else {
        logger = new AccessLogger(config.server.accessLog);
    }
