...
Cache.prototype.getBackendFromHostHeader = function (host, callback) {
    ...
    readFromCache(host, function (rows) {
        ...
        var backendConfig = backends[index].split(';');
        var backend = url.parse(backendConfig[0]);
        backend.id = index; // Store the backend index
        backend.frontend = host; // Store the associated frontend
        if (backendConfig.length === 2) {
            backend.name = backendConfig[1];
        }

        var vhostConfig = virtualHost.split(';');
        backend.virtualHost = vhostConfig[0]; // Store the associated vhost
        if (vhostConfig.length === 2) {
            backend.systemTokenID = vhostConfig[1];
        }
